...
func (model Model) HasPolicy(sec string, ptype string, rule []string) (bool, error) {
	...
	_, ok := model[sec][ptype].PolicyMap.Load(strings.Join(rule, DefaultSep))
	return ok, nil
}
...
func (model Model) AddPolicy(sec string, ptype string, rule []string) error {
	...
	assertion.PolicyMap.Store(strings.Join(rule, DefaultSep), len(assertion.Policy)-1)
	...
	if sec == "p" && hasPriority {
		if idxInsert, err := strconv.Atoi(rule[assertion.FieldIndexMap[constant.PriorityIndex]]); err == nil {
			...
			for ; i > 0; i-- {
				...
				assertion.PolicyMap.Store(strings.Join(assertion.Policy[i-1], DefaultSep), i)
			}
...
func (model Model) RemovePolicy(sec string, ptype string, rule []string) (bool, error) {
	...
	indexVal, ok := ast.PolicyMap.Load(key)
	if !ok {
		return false, nil
	}
	index := indexVal.(int)
...
func (model Model) UpdatePolicy(sec string, ptype string, oldRule []string, newRule []string) (bool, error) {
	...
	indexVal, ok := model[sec][ptype].PolicyMap.Load(oldPolicy)
	if !ok {
		return false, nil
	}
	index := indexVal.(int)
	...
	model[sec][ptype].PolicyMap.Store(strings.Join(newRule, DefaultSep), index)
...
func (model Model) UpdatePolicies(sec string, ptype string, oldRules, newRules [][]string) (bool, error) {
	...
	for oldIndex, oldRule := range oldRules {
		...
		model[sec][ptype].PolicyMap.Delete(oldPolicy)
		model[sec][ptype].PolicyMap.Store(strings.Join(newRules[newIndex], DefaultSep), index)
...
func (model Model) RemovePoliciesWithAffected(sec string, ptype string, rules [][]string) ([][]string, error) {
	...
	var affected [][]string
	for _, rule := range rules {
		indexVal, ok := model[sec][ptype].PolicyMap.Load(strings.Join(rule, DefaultSep))
		if !ok {
			continue
		}
		index := indexVal.(int)
		...
		model[sec][ptype].PolicyMap.Delete(strings.Join(rule, DefaultSep))
		for i := index; i < len(model[sec][ptype].Policy); i++ {
			model[sec][ptype].PolicyMap.Store(strings.Join(model[sec][ptype].Policy[i], DefaultSep), i)
		}
	}
	return affected, nil
}
...
func (model Model) RemoveFilteredPolicy(sec string, ptype string, fieldIndex int, fieldValues ...string) (bool, [][]string, error) {
	...
	for _, rule := range model[sec][ptype].Policy {
		...
		if matched {
			...
			tmp = append(tmp, rule)
			model[sec][ptype].PolicyMap.Store(strings.Join(rule, DefaultSep), len(tmp)-1)
		}
	}
